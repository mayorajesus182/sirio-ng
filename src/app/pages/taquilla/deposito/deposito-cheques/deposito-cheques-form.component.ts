...
export class DepositoChequesFormComponent extends FormBaseComponent implements OnInit {
    ...
    validateSerialAccountUnique(serial: string, numeroCuentaCheque: string) {
        ...
        if (validate) {
            this.depositoService.exists(serial, numeroCuentaCheque).subscribe(data => {
                if (data.exists) {
    ...
    calculateDifferences() {
        ...
        let valorChequePorpio = this.f.chequePropio.value ? this.f.chequePropio.value : 0;
        let valorChequeOtros = this.f.chequeOtros.value ? this.f.chequeOtros.value : 0;
        let valorMontoTotal = this.f.monto.value ? this.f.monto.value : 0;
