...
export class TelefonoFormPopupComponent extends PopupBaseComponent implements OnInit, AfterViewInit {
    ...
    ngOnInit() {
        ...
        this.IdPersona = this.defaults.payload.persona;
    ...
    buildForm() {
        this.itemForm = this.fb.group({
            tipoTelefono: new FormControl({
                value: this.telefono.tipoTelefono || (this.cantidadActual == 0 ? TelefonoConstants.TIPO_TELEFONO_PERSONAL : undefined),
                disabled: this.cantidadActual == 0
            }, [Validators.required]),
            claseTelefono: new FormControl({
                value: this.telefono.claseTelefono || (this.cantidadActual == 0 ? TelefonoConstants.TELEFONO_CELULAR : ''),
                disabled: this.cantidadActual == 0
            }, [Validators.required, Validators.pattern(RegularExpConstants.ALPHA_ACCENTS_SPACE)]),
        ...
        this.f.numero.valueChanges.subscribe(val => {
            if (val) {
                ...
                if (!this.f.numero.invalid) {
                    this.telefonoService.exist(this.IdPersona, val).subscribe(data => {
                        if (data.exists) {
                            this.f.numero.setErrors({exists2: true});
