...
import { componentDestroyed } from '../../../@sirio/shared/component-destroyed';
...
export class SidenavService implements OnDestroy {
  ...
  private getItemByRouteRecursive(route: string, collection: SidenavItem[]) {
    ...
    if (!result) {
      each(collection, (item) => {
        if (item && item.subpermisos && item.subpermisos.length > 0) {
          const found = this.getItemByRouteRecursive(route, item.subpermisos);
  ...
  private setParentRecursive(item: SidenavItem) {
    if (item.subpermisos && item.subpermisos.length > 0) {
      item.subpermisos.forEach(subItem => {
        subItem.parent = item;
